...
There have been some slight changes to how to configure Gmail as the relay recently because Google has depricated its "insecure apps" option. In order to allow sign-in from Gmail using Postfix, you will need to enable "less secure apps" in Google's security settings. Before this, you will need to make sure 2FA is enabled.
...
1. **Important:** To create an app password, you need 2-Step Verification on your Google Account. To enable 2FA, go to your Google accounts [security settings](https://myaccount.google.com/security) and turn on "2-Step Verification".

2. Once 2FA is configure, on the same security settings page you will see the "Less Secure Apps" feature is disabled by defult. Toggle this setting "on".  

3. Now that 2FA is enabled and "less secure apps" is turned on, you can create an **app password** for Postfix. While you are still signed in to your Google account, follow this link to [create and manage your app passwords](https://myaccount.google.com/apppasswords).

4. On the app passowrds page, enter a name for the application you are creating a password for, in this case "Postfix". A 16-digit passcode will be generated. Place this passcode in a secure location for now, as we will need it later on while configuring Postfix.
...
5. By defualt, Postfix listens on port 25 for incomin gmail. If you're running a firewalll on your server, make sure to open port 25 by running the command:
```bash
...
**Note:** Check your spam folder, the email will come from `root` and appear from the sender `<root@localhost.localdomain>`. This is a good sign. It means postfix has been succesfully installed and is working as expected. Next, let's configure the application to sign-in using your Gmail account.
